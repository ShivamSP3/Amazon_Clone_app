...
class AdminServices{
  void sellProducts({
    ...
    try {
      final cloudinary = CloudinaryPublic('df3dxouhu', 'tzy7rkpi');
      List<String> imageUrl =[];

      for (int i = 0; i < images.length; i++) {
       CloudinaryResponse res = await cloudinary.uploadFile(CloudinaryFile.fromFile(images[i].path,
       folder: name
       ));
